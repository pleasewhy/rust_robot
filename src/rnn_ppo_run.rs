...
pub fn train_network<ENV: MujocoEnv + Send + 'static>() {
    ...
    let mut actor_net =
        LstmPolicyConfig::new(action_dim, ob_dim, 256).init::<Autodiff<MyBackend>>(&device);
    ...
    let mut runner =
        on_policy_runner::OnPolicyRunner::<ENV, Autodiff<MyBackend>>::new(device, config, "rnn_ppo");
